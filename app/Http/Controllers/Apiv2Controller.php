...
class Apiv2Controller extends Controller
{
  ...
  /*  public function loginadmin(Request $request)
    ...
    public function loginadmin(Request $request)
    {
        ...
        $adminkoperasi = Adminkoperasi::where('username', $username)->with('koperasi.tipekoperasi','koperasi.kelompokkoperasi')->first();
        ...
        if (!is_null($adminkoperasi)) {
            ...
            if ($adminkoperasi->status == 'Blocked') {
                ...
                if (Hash::check($password, $adminkoperasi->password)) {
						...
						Sessionkoperasi::where('adminkoperasi_id',$adminkoperasi->id)->update(['ip_address' => $ip_address]);
                        //$cekaktifuser->update(['ip_address' => $ip_address]);
						//dd($cekaktifuser);
                        Adminkoperasi::find($adminkoperasi->id)->update(['logingagal' => '0']);
            ...
            return Response::json(['status' => 0, 'message' => 'error...kosong']);
        } else {
            return Response::json(['status' => 0, 'message' => 'adminkoperasi tidak ditemukan']);
        }
    ...
    public function logout(Request $request)
    {
        ...
        Sessionkoperasi::where(['session_key' => $session_key])->update(['status' => 0]);
    ...
    public function getmykoperasi(Request $request)
    {
        ...
        $profile = Koperasi::where('id',$koperasi_id)->with('kelurahan.kecamatan.kabupatenkota.provinsi')->first();
    ...
    public function getprofile(Request $request)
    {
        ...
        $profile = Adminkoperasi::where(['id' => $adminkoperasi_id])->with('koperasi.tipekoperasi')->first();
    ...
    public function updateprofile(Adminkoperasi $adminkoperasi, Request $request)
    {
		...
		$update = Adminkoperasi::find($adminkoperasi->id)->update($input);
        //print_r($update);die();
        ...
        $updated = Adminkoperasi::where(['id' => $adminkoperasi->id])->with('koperasi.tipekoperasi')->first();
    ...
    public function getanggotakoperasi(Request $request)
    {
		...
		$anggotakoperasi = Anggotakoperasi::where(['koperasi_id' => $koperasi_id])->with('kelurahan')->get();
    ...
    public function insertanggotakoperasi(Request $request)
    {
		...
		$checkUsername = Anggotakoperasi::where('username',$input['username'])->first();
		if(!empty($checkUsername)){
			if($checkUsername->count() != 0){
				return Response::json(['status' => 0, 'message' => 'Username sudah ada']);
			}
		}
		...
		if(empty($koperasi)){
			return Response::json(['status' => 0, 'message' => 'koperasi tidak ditemukan']);
		}
		$jumlah = $koperasi['simpananpokok'];
		
		$inputs['koperasi_id'] = $koperasi_id;
		$inputs['anggotakoperasi_id'] = $insert['id'];
		$inputs['jenissimpanan'] = "Pokok";
		$inputs['tanggalbayar'] = Carbon::now();
		$inputs['jumlah'] = $jumlah;
		$inputs['status'] = "Belum Diambil";
		
		$insert2 = Simpanan::create($inputs);
    ...
    public function updateanggotakoperasi(Anggotakoperasi $anggotakoperasi, Request $request)
    {
        ...
        if(!is_null($foto)) {
            ...
            if(file_exists(public_path('images/anggotakoperasi/'.$info->foto))) {
                File::delete(public_path('images/anggotakoperasi/'.$info->foto));
                File::delete(public_path('images/anggotakoperasi/thumb_'.$info->foto));
            }
        ...
        $karyawan = Karyawan::where('anggotakoperasi_id',$anggotakoperasi_id)->get();        
        if(!empty($karyawan)){     
            $koperasi_id=$this->getKoperasiId($session_key);  
            $inputkaryawan['koperasi_id'] = $koperasi_id;   
            $inputkaryawan['id']= $karyawan[0]->id;   
            $inputkaryawan['nama']= $input['nama'];   
            $inputkaryawan['telepon']= $input['telepon'];   
            $inputkaryawan['pendidikan']= $input['pendidikan'];   
            $inputkaryawan['jeniskelamin']= $input['jenis_kelamin']; 
            $inputkaryawan['alamat']= $input['alamat']; 
            $inputkaryawan['kelurahan_id']= $input['kelurahan_id']; 
            $updatekaryawan = Karyawan::where('anggotakoperasi_id',$anggotakoperasi_id)->update($inputkaryawan);       
        }                     
        $hasil = Anggotakoperasi::where(['id' => $anggotakoperasi_id])->first();        
        return Response::json(['status' => 1, 'message' => 'sukses update', 'session_key' => $session_key, 'data' => $hasil]);
    }
    ...
    public function getinfokoperasi(Request $request)
    {
		...
		$dt = Carbon::now();
        $infokoperasi = Infokoperasi::where(['koperasi_id' => $koperasi_id])->where('tanggalmulai','<=',$dt)
																			->where('tanggalselesai', '>=', $dt)->get();
    ...
    /*------------  insert rat  ----------------*/
    public function insertrat(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
    ...
    /*------------  edit myrat ----------------*/
    public function editrat (Request $request)
    {
        $session_key = $request->input('session_key');
        $rat_id= $request->input('rat_id');

        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        }

        $rat_id=Rat::find($rat_id);
        ...
        return Response::json(['status' => 1, 'message' => 'data ditemukan',
            'session_key' => $session_key, 'data' => $rat_id]);
    ...
    /*------------  Update rat  ----------------*/
    public function updaterat (Request $request)
    {
        $session_key = $request->input('session_key');
        $rat_id= $request->input('rat_id');
        ...
        $rat=Rat::find($rat_id);
        if(is_null($rat)){
            return Response::json(['status' => 0, 'message' => 'RAT tidak ditemukan']);
        }

        $input = $request->all();

        $update= Rat::find($rat_id)->update($input);
    ...
    /*------------  delete delete RAt  ----------------*/
    public function deleterat (Request $request)
    {
        $session_key = $request->input('session_key');
        $rat_id= $request->input('rat_id');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
    ...
    /*------------  get kehadiran rat aktif ----------------*/
    public function getkehadiranrataktif(Request $request)
    {
        $session_key = $request->input('session_key');
        ...
        if (is_null($tahunoperasiaktif)) {
            return Response::json(['status' => 0, 'message' => 'data belum tersedia']);
        }


        $cekrat = Rat::where('tahunoperasi_id',$tahunoperasiaktif->id)->first();
        ...
        $rat = Kehadiranrat::with('rat')->with('anggotakoperasi')->where(['kehadiranrat.rat_id' => $cekrat->id])->orderBy('kehadiranrat.id', 'desc')->get();
    ...
    public function getbookingtrainingkoperasi(Request $request)
    {
        ...
        $training = DB::table('trainingkoperasi')->leftjoin('bookingtrainingkoperasi','bookingtrainingkoperasi.trainingkoperasi_id','=','trainingkoperasi.id')
												 ->leftjoin(DB::raw('(SELECT id,trainingkoperasi_id,anggotakoperasi_id, count(anggotakoperasi_id) AS jmlbuking FROM bookingtrainingkoperasi GROUP BY bookingtrainingkoperasi.trainingkoperasi_id) as v'),'v.trainingkoperasi_id','=','trainingkoperasi.id')
												 ->where('trainingkoperasi.koperasi_id',$koperasi_id)->groupby('trainingkoperasi.id')
												 ->get(['*']);
    ...
    public function getbookingtrainingkoperasiwhere(Request $request)
    {
        ...
        $training=DB::table('bookingtrainingkoperasi')
			...
			->leftjoin('trainingkoperasi', 'trainingkoperasi.id','=','bookingtrainingkoperasi.trainingkoperasi_id')
            ->where('trainingkoperasi_id',$trainingkoperasi_id)
            ->orderby('anggotakoperasi_id','desc')
            ->get();

		$data = DB::table('bookingtrainingkoperasi')->join('notifikasi','bookingtrainingkoperasi.id','=','notifikasi.bookingtrainingkoperasi_id')
													->where('bookingtrainingkoperasi.trainingkoperasi_id',$trainingkoperasi_id)->select('notifikasi.id')->get();
		
		if(!empty($data)){
			foreach($data as $dat){
				$datx[] = $dat->id;
			}
			$this->updatenotifikasi($datx);
		}
    ...
    public function getbookingseminarkoperasi(Request $request)
    {
        ...
        $semiar=DB::select('call sp_getBookingSeminarKoperasi(?)', [$koperasi_id]);
    ...
    public function getbookingseminarkoperasiwhere(Request $request)
    {
        ...
        $seminar=DB::table('bookingseminarkoperasi')->leftjoin('anggotakoperasi','anggotakoperasi.id','=','bookingseminarkoperasi.anggotakoperasi_id')
													->where('seminarkoperasi_id',$seminarkoperasi_id)
													->orderby('anggotakoperasi_id','desc')->get();

		$data = DB::table('bookingseminarkoperasi')->join('notifikasi','bookingseminarkoperasi.id','=','notifikasi.bookingseminarkoperasi_id')
												   ->where('bookingseminarkoperasi.seminarkoperasi_id',$seminarkoperasi_id)->select('notifikasi.id')->get();
		if(!empty($data)){
			foreach($data as $dat){
				$dats[] = $dat->id;
			}
			$this->updatenotifikasi($dats);
		}
    ...
    public function gettahunoperasi(Request $request)
    {
        ...
        $infokoperasi = Tahunoperasi::where(['koperasi_id' => $koperasi_id])->orderBy('status', 'asc')->get();
    ...
    public function getsimpanan(Request $request)
    {
        ...
        $infokoperasi = Simpanan::with('anggotakoperasi')->where(['koperasi_id' => $koperasi_id, 'jenissimpanan' => "Sukarela"])->orderBy('simpanan.id', 'desc')->get();
    ...
    public function deletepembelian(Request $request)
    {
        ...
        $del= Pembelian::where('id',$pembelian_id)->delete();
    ...
    public function kalkulasishu(Request $request)
    {
		...
		if(empty($komponenshujasaanggota)){
			return Response::json(['status' => 0, 'message' => 'Komponen SHU jasa anggota tidak ada']);
		}
		...
		if(empty($komponenshujasamodal)){
			return Response::json(['status' => 0, 'message' => 'Komponen SHU jasa modal tidak ada']);
		}
    ...
    public function getnewpesankementerian (Request $request)
    {
		...
		$pesan = DB::select('call sp_getNotifikasi(?)', [$koperasi_id]);
		
		//print_r($pesan); die();
		if(array_key_exists('status', $pesan[0])){
			return Response::json(['status' => 0, 'message' => 'data tidak ditemukan']);
		}else{
			return Response::json(['status' => 1, 'message' => 'data ditemukan', 'data' => $pesan]);
		}
    ...
    public function deletepesankementerian(Request $request)
    {
        ...
        $del= Pesankementeriankoperasi::where('id',$pesankementerian_id)->delete();
    ...
    public function showpesertatrainingkementerian(Request $request)
    {
		...
		$data = DB::table('trainingkementerian')->where('trainingkementerian.id',$trainingkementerian_id)->join('bookingtrainingkementerian','trainingkementerian.id','=','bookingtrainingkementerian.trainingkementerian_id')
																										 ->join('notifikasi','bookingtrainingkementerian.id','=','notifikasi.bookingtrainingkementerian_id')
																										 ->select('trainingkementerian.id as tid','bookingtrainingkementerian.id as bid','notifikasi.id as nid')->get();
		if(!empty($data)){
			foreach($data as $value){
				$datas[] = $value->nid;
			}
			$update = $this->updatenotifikasi($datas);
		}
	...
	/*------------  Delete Pembelian ----------------*/
	private function updatenotifikasi($no){
		
		$id = $no;
		
		if($id != null){
			foreach($id as $idx){
				$data = Notifikasi::where('id',$idx)->update(['status' => "Dibaca"]);
			}
		}
    ...
    public function showpesertaseminarkementerian(Request $request)
    {
		...
		$data = DB::table('seminarkementerian')->where('seminarkementerian.id',$seminarkementerian_id)->join('bookingseminarkementerian','seminarkementerian.id','=','bookingseminarkementerian.seminarkementerian_id')
																											->join('notifikasi','bookingseminarkementerian.id','=','notifikasi.bookingseminarkementerian_id')
																											->select('seminarkementerian.id as sid','bookingseminarkementerian.id as bid','notifikasi.id as nid')->get();
		if(!empty($data)){
			foreach($data as $value){
				$datas[] = $value->nid;
			}
    ...
    public function canceltrans(Request $request)
    {
        ...
        return Response::json(['status' => 1, 'message' => 'data ditemukan', 'session_key' => $session_key,'deletetrans'=>$deletetrans]);
    }
    ...
    public function pembayaran (Request $request)
    {
        ...
        return Response::json(['status' => 1, 'message' => 'data ditemukan', 'session_key' => $session_key,'data'=>$newstatustrans]);
    }

    /* ------------- TAmbahan seminar kementerian */
    public function registrasikoperasi(Request $request)
    {
        $input = $request->all(); 
        $input['foto'] = 'no_image.png';
        $insert = Koperasi::create($input);        
        if (!$insert) {
            return Response::json(['status' => 0, 'message' => 'Insert Koperasi Gagal']);
        }
        else
        {
            $inputadmin['koperasi_id'] = $insert->id;
            $inputadmin['username'] = $request->input('username');
            $inputadmin['nama'] = $request->input('nama');
            $inputadmin['email'] = $request->input('email');
            $inputadmin['telepon'] = $request->input('telepon');
            $inputadmin['foto'] = 'no_image.png';
            $inputadmin['status'] = 'Registered';
            $hash = $request->input('password');
            $inputadmin['password'] = Hash::make($hash);
            $inputadmin['akseskoperasi_id'] = 1;         
            $insertadmin = Adminkoperasi::create($inputadmin);
            if (!$insertadmin) {
                return Response::json(['status' => 0, 'message' => 'Insert Admin Koperasi Gagal']);
            }   
        }
        return Response::json(['status' => 1, 'message' => 'insert berhasil', 'data' => $insertadmin]);
    }

    public function getkelompokkop(Request $request)
    {
        $get = Kelompokkoperasi::get();
        if (is_null($get)) {
            return Response::json(['status' => 0, 'message' => 'data tidak ditemukan']);
        }
        return Response::json(['status' => 1, 'message' => 'data ditemukan', 'data' => $get]);
    }  
    ...
    {
        $koperasi = Koperasi::with('kelurahan.kecamatan.kabupatenkota.provinsi','tipekoperasi')
                    ->orderBy('koperasi.id', 'asc')
                    ->get();
        if (is_null($koperasi)) {
            return Response::json(['status' => 0, 'message' => 'data tidak ditemukan']);
        }
        return Response::json(['status' => 1, 'message' => 'data ditemukan', 'data' => $koperasi]);
    }  

    public function insertkoperasisekunder(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        try {
            $input = $request->all();
            $input['sekunder_id'] = $koperasi_id;
            $insert = Sekunderkoperasi::Create($input);
        }
        catch (\Exception $e) {
        ...
        if (is_null($insert)) {
            return Response::json(['status' => 0, 'message' => 'insert data gagal']);
        }
        return Response::json(['status' => 1, 'message' => 'insert data berhasil']);
    }  

    public function getkoperasisekunder(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $get = Sekunderkoperasi::where(['sekunder_id' => $koperasi_id, 'status' => 'Aktif'])->with('koperasi')->get();
        if (is_null($get)) {
            return Response::json(['status' => 0, 'message' => 'data tidak di temukan']);
        }
        return Response::json(['status' => 1, 'message' => 'data di temukan', 'session_key' => $session_key, 'data' => $get]);
    }  

    public function keluarkoperasisekunder(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $koperasisekunder_id = $request->input('koperasisekunder_id');
        $update = Sekunderkoperasi::where(['id' => $koperasisekunder_id])->update(['status' => 'Tidak Aktif']);

        if (is_null($update)) {
            return Response::json(['status' => 0, 'message' => 'data tidak di temukan']);
        }
        return Response::json(['status' => 1, 'message' => 'Berhasil di Update']);
    } 

    public function updatekelengkapanadmin(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $admkop = Administrasikoperasi::where('koperasi_id', $koperasi_id)->first();
        $input = $request->all();
        $dokumen = Input::file('docs');
        
        if (empty($admkop)) {            
            if (!is_null($dokumen)) {            
                $filename = date('YmdHis') . '.' . $dokumen->getClientOriginalExtension();
                $path = public_path('document/administrasikoperasi/');
                $dokumen->getSize();
                $dokumen->move($path, $filename);      
                $input['dokijinsimpanpinjam'] = $filename;
                $input['koperasi_id'] = $koperasi_id;
                $insert = Administrasikoperasi::Create($input);      
            }
            else {                
                return Response::json(['status' => 0, 'message' => 'Dokumen upload tidak di temukan']);
            }            
        }
        else {
            File::delete(public_path('document/administrasikoperasi/' . $admkop->dokijinsimpanpinjam));
            if (!is_null($dokumen)) {            
                $filename = date('YmdHis') . '.' . $dokumen->getClientOriginalExtension();
                $path = public_path('document/administrasikoperasi/');
                $dokumen->getSize();
                $dokumen->move($path, $filename);      
                $update['dokijinsimpanpinjam'] = $filename;
                $update['visi'] = $input['visi'];
                $update['misi'] = $input['misi'];
                $update['tujuan'] = $input['tujuan'];
                $update['status'] = 'Aktif';
                $insert = Administrasikoperasi::where(['koperasi_id' => $koperasi_id])->update($update);      
            }
            else {                
                return Response::json(['status' => 0, 'message' => 'Dokumen upload tidak di temukan']);
            }     
        ...
        if (!$insert) {
            return Response::json(['status' => 0, 'message' => 'Insert profile gagal']);
        }
        return Response::json(['status' => 1, 'message' => 'sukses', 'data' => $insert]);
    }    

    public function getanggotabermasalah(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $get = Anggotakoperasi::where(['status' => 'Blacklist'])->with('koperasi')->get();
        if (is_null($get)) {
            return Response::json(['status' => 0, 'message' => 'data tidak di temukan']);
        }
    ...
    public function insertanggotabermasalah(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $anggotakoperasi_id = $request->input('anggotakoperasi_id');
        $update = Anggotakoperasi::where(['id' => $anggotakoperasi_id])->update(['status' => 'Blacklist']);

        if (is_null($update)) {
            return Response::json(['status' => 0, 'message' => 'Update Data Gagal']);
        }
        return Response::json(['status' => 1, 'message' => 'Update berhasil']);
    } 

    public function hapusanggotabermasalah(Request $request)
    {
        $session_key = $request->input('session_key');
        if ($this->checkIfSessionkoperasiExpired($session_key) == false) {
            return Response::json(['status' => 0, 'message' => 'session key tidak ditemukan']);
        ...
        $anggotakoperasi_id = $request->input('anggotakoperasi_id');
        $update = Anggotakoperasi::where(['id' => $anggotakoperasi_id])->update(['status' => 'Registered']);
