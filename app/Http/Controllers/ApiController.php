...
class ApiController extends Controller
{
    ...
    public function auth(Request $request)
    {
        ...
        //$anggotakoperasi = Anggotakoperasi::where('email', $email)->orWhere('username', $username)->first();
		
		$anggotakoperasi = Anggotakoperasi::where('username', $username)->first();
            ...
            if ($anggotakoperasi->status == 'Blocked') {
                ...
                if (Hash::check($password, $anggotakoperasi->password)) {
                    ...
                    if (is_null($cekaktifuser)) {
                        ...
                        $this->firstLogin($anggotakoperasi->id);

                        Session::where(['anggotakoperasi_id' => $anggotakoperasi->id])->update(['unik_id' => $unik_id]);
                        Anggotakoperasi::find($anggotakoperasi->id)->update(['logingagal' => '0']);
						$anggotakoperasi1 = Anggotakoperasi::find($anggotakoperasi->id);
                        $session_key = $this->createOrUpdateSession($anggotakoperasi->id);

                        return Response::json(['status' => 1, 'message' => 'adminkoperasi ditemukan', 'statuslogin' => '1', 'session_key' => $session_key, 'data' => $anggotakoperasi1]);
    ...
    public function logout(Request $request)
    {
        ...
        Session::where(['session_key' => $session_key])->update(['status' => 0]);
...
//$desa = \App\Desa::where('nama', 'like', '%sukamakmur%')->with('kecamatan.kabupaten.provinsi')->get();
    ...
    public function getinfokementerian(Request $request)
    {
		...
		$dt = Carbon::now();
        $infokementerian = Infokementerian::where('tanggalmulai', '<=', $dt)
											->where('tanggalselesai', '>=', $dt)->get();

        if ($infokementerian->isEmpty()) {
            return Response::json(['status' => 0, 'message' => 'data tidak ditemukan']);
        }
    ...
    public function getinfokoperasi(Request $request)
    {
		...
		$dt = Carbon::now();
        $infokementerian = Infokoperasi::where('koperasi_id',$koperasi_id)->where('tanggalmulai', '<=', $dt)
											->where('tanggalselesai', '>=', $dt)->get();
    ...
    public function getrat(Request $request)
    {
        ...
        $rat = Rat::where(['koperasi_id' => $koperasi_id,'tahunoperasi_id'=>$tahunoperasiaktif->id])->first();
    ...
    public function insertbookingtrainingkoperasi(Request $request)
    {
		...
		$data = array('koperasi_id' => $koperasi_id,'anggotakoperasi_id' => $anggotakoperasi_id,'bookingtrainingkoperasi_id' => $insert['id']);
		$insert2 = Notifikasi::create($data);
    ...
    public function insertbookingseminarkoperasi(Request $request)
    {
		...
		$koperasi = Anggotakoperasi::where('id',$anggotakoperasi_id)->select('koperasi_id')->first();
		...
		$data = array('koperasi_id' => $koperasi_id,'anggotakoperasi_id' => $anggotakoperasi_id, 'bookingseminarkoperasi_id' => $insert['id']);
		$insert2 = Notifikasi::create($data);
    ...
    public function insertbookingtrainingkementerian(Request $request)
    {
		...
		$data = array('koperasi_id' => $koperasi_id,'anggotakoperasi_id' => $anggotakoperasi_id,'bookingtrainingkementerian_id' => $insert['id']);
		$insert2 = Notifikasi::create($data);
    ...
    public function insertbookingseminarkementerian(Request $request)
    {
		...
		$data = array('koperasi_id' => $koperasi_id, 'anggotakoperasi_id' => $anggotakoperasi_id, 'bookingseminarkementerian_id' => $insert['id']);
		$insert2 = Notifikasi::create($data);
    ...
    public function getinfokoperasidetail(Request $request)
    {
		...
		//$insert = Viewinfokoperasi::create('infokoperasi_id'=>$infokoperasi_id);
		try{
			$insert = Viewinfokoperasi::create(['infokoperasi_id' => $infokoperasi_id, 'anggotakoperasi_id' => $anggotakoperasi_id, 'status' => 'Belum Dibaca']);
		}catch(\Exception $e){
			$insert = 0;
		}
  ...
  // $infokoperasi = Anggotakoperasi::get(['id','nama']);
    ...
    public function getshu(Request $request)
    {
		...
		if(empty($tahunoperasiaktif)){
			return Response::json(['status' => 0, 'message' => 'Belum ada tahun operasi']);
		}
