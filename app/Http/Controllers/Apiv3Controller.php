...
class Apiv3Controller extends Controller
{
    ...
    public function logout(Request $request)
    {
        ...
        //
        $sess=Sessionkementerian::where(['session_key' => $session_key])->first();
        if(!empty($sess)){
            $sess->update(['status' => 0,'expired_at'=>Carbon::now()]);
            $sess->save();
        }
        else
        {
            return Response::json(['status' => 0, 'message' => 'logout gagal']);
        }
        //
    ...
    public function getkoperasi(Request $request)
    {
        ...
        $koperasi = Koperasi::with('kelurahan.kecamatan.kabupatenkota.provinsi','tipekoperasi')->orderBy('koperasi.id', 'desc')->get();
    ...
    public function getsuperadmin(Request $request)
    {
        ...
        $koperasi=DB::select('call sp_getKoperasiTanpaAdmin()');
    ...
    public function editkoperasi(Request $request)
    {
        ...
        $profile = Koperasi::with('kelurahan.kecamatan.kabupatenkota.provinsi')->find($koperasi_id);
    ...
    public function insertpesankementerian(Request $request)
    {
		...
		$data = array('koperasi_id' => $input['koperasi_id'], 'pesankementeriankoperasi_id' => $insert['id']);
    ...
    public function gettrainingkementerian(Request $request)
    {
        ...
        $training = DB::select('call sp_getTrainingKementerian');
    ...
    public function edittrainingkementerian (Infokementerian $infokementerian,Request $request)
    {
		...
		$info['daftarpeserta'] = Bookingtrainingkementerian::where('status', 'Approved')->with('koperasi','anggotakoperasi')->get();
    ...
    public function getseminarkementerian(Request $request)
    {
		...
		$seminar = DB::select(' call sp_getSeminarKementerian');
    ...
    public function editseminarkementerian (Request $request)
    {
		...
		$info['daftarpeserta'] = Bookingseminarkementerian::where('status','Approved')->with('anggotakoperasi','koperasi')->get();
        if(!$info){
            return Response::json(['status' => 0, 'message' => 'Update gagal']);
        }
